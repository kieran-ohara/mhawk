...
const getPaymentPlansActiveFromDate = async (startDate) => {
  const result = await getPaymentPlans((params) => {
    return `
        WHERE (
          ${params.plansTableName}.start_date >= '${mysqlDate(startDate)}'
        )
        `;
  });
  return result;
...
const getPaymentPlansActiveForPointInTime = async (forDate) => {
  ...
  plans.filter((plan) => {
    ...
    return isWithinInterval(forDate, { start, end });
  });
  return plans;
};
