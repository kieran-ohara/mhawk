...
WHERE plans.end_date IS NOT NULL
GROUP BY plans.id
`);
...
const addPaymentPlans = async (opts) => {
...
const addMonthlyPayments = async (recurringPayments) => {
  const result = await query(
    'INSERT INTO payment_plans (monthly_price, start_date, reference, is_shared) VALUES ?',
    [
...
const getMonthlyPaymentsForDate = async (date) => {
...
WHERE (DATE('${dateAsString}') BETWEEN plans.start_date AND plans.end_date) OR plans.end_date IS NULL;
  `);
  return result;
};
