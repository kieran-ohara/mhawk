...
const getPaymentPlans = async (whereFilterFn) => {
  ...
  whereClause = whereClause.concat(' plans.deleted_at IS NULL');
...
const getPaymentPlansWithTag = async (slug) => {
  return getPaymentPlans((params) => {
    ...
    WHERE ${params.plansTableName}.id IN (
      SELECT jt.payment_plan_id FROM tags INNER JOIN payment_plan_tags AS jt ON jt.tag_id = tags.id WHERE tags.slug = '${slug}' AND jt.deleted_at IS NULL
    )
    `;
  });
};
