...
import { getAllPaymentPlans } from "./payment-plans";
import { getTaxedWage } from "./user";

const getChartDataForMonths = async (startDate, endDate) => {
  const plans = await getAllPaymentPlans(startDate);
...
const aggregateSum = (plans, filter) => {
  return plans
    .filter(filter)
    .reduce((acc, plan) => acc + plan.monthly_price, 0);
};
...
const getChartDataForMonthsAggregateEndDate = async (startDate, endDate) => {
  ...
  const data = months.map((month) => {
    ...
    return {
      name: format(month, "LLL y"),
      PaymentPlans: sumRecurring,
      Subscriptions: sumFinite,
